...
def conv2d(input_, output_dim, 
           k_h=5, k_w=5, d_h=1, d_w=1, stddev=0.35,
           name="conv2d"):
    with tf.variable_scope(name):
...
def linear(input_, output_size, scope=None, stddev=0.35, bias_start=0.01, with_w=False):
...
class baseCNN():
	def __init__(self, numActions):
		...
		conv0 = lrelu(c_bn0(conv2d(self.state, 32, k_h=7, k_w=7, d_h=2, d_w=2,  name="c_conv0")))
		conv1 = lrelu(c_bn1(conv2d(conv0, 64, k_h=4, k_w=4, d_h=2, d_w=2, name="c_conv1")))
		conv2 = lrelu(c_bn2(conv2d(conv1, 64, k_h=4, k_w=4, name="c_conv2")))
		...
		fc1 = lrelu(c_bn3(linear(flattened, 512, 'c_fully_connected')))
		self.out = tf.nn.sigmoid(c_bn4(linear(fc1, self.numActions, 'c_out_layer')))

		self.q = tf.nn.softmax(self.out)

		# Target Network
		# ---------------------------------------
		t_bn0 = batch_norm(name='t_bn0')
		t_bn1 = batch_norm(name='t_bn1')
		t_bn2 = batch_norm(name='t_bn2')
		t_bn3 = batch_norm(name='t_bn3')
		t_bn4 = batch_norm(name='t_bn4')

		t_conv0 = lrelu(t_bn0(conv2d(self.target_state_t, 32, k_h=7, k_w=7, d_h=2, d_w=2,  name="t_conv0")))
		t_conv1 = lrelu(t_bn1(conv2d(t_conv0, 64, k_h=4, k_w=4, d_h=2, d_w=2, name="t_conv1")))
		t_conv2 = lrelu(t_bn2(conv2d(t_conv1, 64, k_h=4, k_w=4, name="t_conv2")))
		# conv3 = lrelu(c_bn4(conv2d(conv2, 32, k_h=7, k_w=7, name="c_conv3")))
		# flattened = tf.reshape(conv2, [self.batch_size, -1])
		target_shape = t_conv2.get_shape().as_list()[1:]
		target_newshape = 1
		for dimension in target_shape:
			target_newshape = target_newshape*dimension
		target_flattened = tf.reshape(t_conv2, [-1, target_newshape])
		print(target_flattened.get_shape())
		target_fc1 = lrelu(t_bn3(linear(target_flattened, 512, 't_fully_connected')))
		self.targetout = tf.nn.sigmoid(t_bn4(linear(target_fc1, self.numActions, 't_out_layer')))

		self.target_q = tf.nn.softmax(self.targetout)
